			...
			<div class="wrapper indent">
				...
				<section id="content">
					<div id="slogan">
						...
						<div class="side_nav">
							<ul>
								<li class="select"><a href="columnAction.action?currentPage=1" name="all">全部分类</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=1" name="mobile">移动开发</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=2" name="web">Web前端</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=3" name="enterprise">架构设计</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=4" name="code">编程语言</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=5" name="www">互联网</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=6" name="database">数据库</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=7" name="system">系统运维</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=8" name="cloud">云计算</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=9" name="software">研发管理</a></li>
								<li><a href="columnAction!showCategoryArticle.action?categoryId=10" name="other">综合</a></li>
							</ul>
						</div>
				...
				<div class="blogcenter">
					<h1 class="specialColumnTop">推荐专栏</h1>
					<div class="specialColumn" align="center">
					
					<% for(BlogColumn bc : blogColumns) {%>
						<dl>
							<dt>
								<a href="columnAction!viewColumnBlog.action?columnId=<%= bc.getColumnId()%>"><img src="images/columnimg.jpg"></a><a
									href="userAction!viewOtherSpace.action?userId=<%=bc.getUser().getUserId() %>&currentPage=1"><%= bc.getUser().getUserName() %></a>
							</dt>
							<dd>
								<a href="columnAction!viewColumnBlog.action?columnId=<%= bc.getColumnId()%>"><%= bc.getColumnName() %></a><br>
								<% if(bc.getColumnIntro().length() > 30) {%>
								<%= bc.getColumnIntro().substring(0,30) %>
								<%}else {%>
								<%= bc.getColumnIntro() %>
								<%} %>
							</dd>
					...
					<div>
						<h3 class="bloglisthead">专栏最新文章</h3>
					</div>

					
					<% for(Article article : articles){
						if(article.getBlogColumn() != null){
					%>
					<div class="bloglist">
						<h1>
							<a class="category" href="columnAction!viewColumnBlog.action?columnId=<%= article.getBlogColumn().getColumnId()%>">【<%= article.getBlogColumn().getColumnName()%>】</a>
							<a class="blogtitle" target="_blank" href="articleAction!viewArticle.action?id=<%= article.getArticleId()%>"><%=article.getTitle() %></a>
						</h1>
						<dl>
							<dt>
						...
						<div class="aboutinfo">
							...
							<a href="userAction!viewOtherSpace.action?userId=<%=article.getUser().getUserId() %>&currentPage=1" target="_blank" class="username"><%= article.getUser().getUserName() %></a> <span>专栏：</span> 
								<a href="columnAction!viewColumnBlog.action?columnId=<%=article.getBlogColumn().getColumnId()%>"><%=article.getBlogColumn().getColumnName() %></a> <span>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=article.getCreateTime() %></span> 
